...
**Spec:** `docs/SPEC.md` (v1.0, decisions locked)
...
|---|---|---|---|---|
| K-01 | Repo scaffold: package layout, `pyproject`, MIT + ODbL, CI, Makefile | Tech Lead | S | ✅ done |
| K-02 | `schema.sql` — entities, aliases, matches, appearances, team_seasons, transfers, honours, facts, source_runs, snapshot_meta | Tech Lead | M | ✅ done |
| K-03 | `normalize.py` — Arabic folding, Latin transliteration, fused-article strip, `xkey()` cross-script skeleton | Identity | M | ✅ done |
| K-04 | `resolver.py` — provider-id → name → cross-script → fuzzy → provisional; alias learning | Identity | L | ✅ done |
| K-05 | `store/db.py` — SQLite store implementing the resolver interface + review queue | Tech Lead | M | ✅ done |
| K-06 | Adversarial identity suite (namesakes, scripts, provisional) | QA | M | ✅ 9 passing |

**Exit gate (day 4):** acceptance items 1–5 pass; CI green on `main`.

### Wave 1 — First real collector (days 4–7)
| # | Item | Owner | Size |
|---|---|---|---|
| K-07 | Collector base: interface, per-source rate budget, `source_runs` writes, graceful-empty contract | Data Eng | M |
| K-08 | 365Scores adapter — fixtures/results via `competitions=` + date window (Saudi id **649**) | Data Eng | M |
| K-09 | Ingest pipeline: raw record → resolver → `matches` upsert (forward-only status) | Data Eng + Tech Lead | M |
| K-10 | Lineups → `appearances` rows (`/web/game/?gameId=&withLineups=true`, `members[]`) | Data Eng | M |
| K-11 | Competition + season seeding for the Saudi league (entities for clubs from `/standings`) | Identity | S |

### Wave 2 — Derivation + read surface (days 7–10)
| # | Item | Owner | Size |
|---|---|---|---|
| K-12 | `derive.form(team, n)` — last-N results computed from `matches` | Tech Lead | S |
| K-13 | `derive.h2h(a, b)` — past meetings + W/D/L record from `matches` | Tech Lead | S |
| K-14 | `derive.squad(team, season)` + `derive.career(player)` from `appearances` | Tech Lead | M |
| K-15 | Read API skeleton (FastAPI): `/v1/search`, `/v1/teams/{id}`, `/v1/matches`, `/v1/h2h` | API Eng | L |
| K-16 | Bilingual response shape + `?lang=ar\|en` display selection | API Eng | S |

### Wave 3 — Snapshot + hardening (days 10–12)
| # | Item | Owner | Size |
|---|---|---|---|
| K-17 | `make snapshot` — export SQLite, stamp `snapshot_meta` (version, generated_at, license, counts) | Data Eng | M |
| K-18 | Snapshot round-trip test — fresh process, identical answers | QA | S |
| K-19 | Review-queue CLI: list provisional entities, merge into canonical, record the alias | Identity | M |
| K-20 | README quickstart verified on a clean clone; `docs/data-dictionary.md` | PO + Tech Lead | S |
| K-21 | Ops: cron layout + monthly snapshot workflow **stub** (wired for real in Sprint 3) | Data Eng | S |
...
6. **PR opened** referencing the item (`Closes K-NN`), reviewed by Tech Lead.
7. **CI green** on the PR (the merge gate).
8. Merged to `main`; branch deleted.
9. Anything touching identity additionally passes the full adversarial namesake suite.
